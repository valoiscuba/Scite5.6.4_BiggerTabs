...
/*cambio*/
private:
	WNDPROC fnOldTabProc = NULL;
	static LRESULT CALLBACK TabSubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
	HFONT hFontTabActive = NULL;
	HFONT hFontTabInactive = NULL;

	COLORREF backActive = RGB(0, 0, 0);
	COLORREF foreActive = RGB(0xDD, 0xDD, 0xDD);

	COLORREF backInactive = RGB(0x11, 0, 0);
	COLORREF foreInactive = RGB(0xFF, 0, 0);

	// Color del área NO cubierta por ningún tab (lo que antes se veía como
	// "fondo detrás de las pestañas"). Configurable con buffers.detras.
	COLORREF backBehindTabs = RGB(0x2D, 0x2D, 0x2D);
	// Color del borde de cada pestaña (solo arriba + derecha).
	// Configurable con buffers.borde.
	COLORREF tabBorderColour = RGB(50, 50, 50);

	void DrawTabItem(LPDRAWITEMSTRUCT pdis);

	// Recalcula ancho/alto reales de las pestañas según las fuentes activas y
	// los títulos actuales de los buffers, y fuerza a Win32 a redistribuirlas.
	void RecalcTabItemSize();

	// Última altura/ancho por pestaña calculados por RecalcTabItemSize().
	// bands[bandTab].height (en SizeSubWindows(), SciTEWinBar.cxx) se
	// recalcula solo, vía TabCtrl_AdjustRect(), a partir de las métricas
	// reales del control una vez que RecalcTabItemSize() las actualiza —
	// estos dos miembros son solo para diagnóstico/log, no hace falta
	// asignarlos a mano en ningún otro lado.
	int tabBarCalculatedWidth = 250;
	int tabBarCalculatedHeight = 32;
